...
const ExamEdit = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const exam = getData(examRes);
        const sessionsPayload = getData(sessionsRes);
        ...
        const existingSession = sessionsList.find((s) => s._id === examSessionId);

        if (existingSession) {
          setTerms(existingSession.terms || []);
        }
        ...
        setForm({
          ...
          showResultAfter: exam.showResultAfter ?? false,
        });

        setSelectedQuestions(Array.isArray(exam.questions) ? exam.questions : []);
      } catch (err) {
        console.error("Load exam edit error:", err);
  ...
  useEffect(() => {
    ...
    const loadClasses = async () => {
      try {
        ...
        const classesPayload = getData(res);
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        const banksRes = await api.get("/cbt/banks", {
          params: {
        ...
        const banksPayload = getData(banksRes);
        const banks = Array.isArray(banksPayload) ? banksPayload : [];
        ...
        const perBank = await Promise.all(
          banks.map((bank) =>
            api
              ...
              .then((r) => {
                const questionsPayload = getData(r);
                const questions = Array.isArray(questionsPayload)
                  ? questionsPayload
                  : [];
  ...
  const selectAll = () => {
    const filtered = filteredQuestions();
    const allPicked =
      filtered.length > 0 && filtered.every((q) => isSelected(q._id));
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-2">
            Start/End = window students can begin. Duration = how long they get
            once they start.
          </p>
        </div>
            ...
            {[
              {
                ...
                desc: "Each student gets questions in a different order",
              },
              {
                key: "shuffleOptions",
                label: "Shuffle options",
                desc: "Options A-D are randomised per student",
              },
              {
                key: "showResultAfter",
                label: "Show result immediately",
                desc: "If off, teacher manually releases results",
              },
            ].map(({ key, label, desc }) => (
              <div key={key} className="flex items-start gap-3">
                <div
        ...
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h2 className="font-semibold text-gray-700">Questions</h2>
              <p className="text-xs text-gray-400 mt-0.5">
                {selectedQuestions.length} selected · {totalMarks} total mark
                {totalMarks !== 1 ? "s" : ""}
              </p>
            </div>
          ...
          {form.subject && form.classId && showQuestionPicker && (
            <div className="border rounded-lg overflow-hidden mb-4">
              <div className="p-3 bg-gray-50 border-b flex flex-wrap gap-2">
                ...
                >
                  {filteredQuestions().length > 0 &&
                  filteredQuestions().every((q) => isSelected(q._id))
                    ? "Deselect all"
                    : "Select all"}
                </button>
              </div>

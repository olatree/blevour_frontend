...
export default function ResultEntryPage() {
  ...
  const isAcademicUnitLocked =
  !isFullAccess && Boolean(restrictedUnitName);
...
useEffect(() => {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      //   res = await api.get(
      //     `/teacher-assignments/${user._id}/subjects`,
      //     {
      //       params: {
      ...
      if (user.role === "teacher" || user.role === "class_teacher") {
          if (!userId) {
            throw new Error("Teacher ID is missing from the logged-in user.");
          }

          res = await api.get(`/teacher-assignments/${userId}/subjects`, {
            params: {
              academicUnitId: selectedAcademicUnit,
            },
          });
      ...
      } else if (Array.isArray(res.data?.data?.subjects)) {
        subjectList = res.data.data.subjects;
      }
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-gray-300 bg-white p-2 text-sm disabled:bg-gray-100"
              >
                <option value="">
                  {loadingSubjects ? "Loading subjects..." : "-- Subject --"}
                </option>

...
export default function PublishResultsPage() {
  ...
  const [selectedAcademicUnitId, setSelectedAcademicUnitId] = useState("");
  const [selectedSessionId, setSelectedSessionId] = useState("");
  const [selectedTermId, setSelectedTermId] = useState("");
  ...
  const selectedAcademicUnit = useMemo(
    () => academicUnits.find((u) => u._id === selectedAcademicUnitId),
    [academicUnits, selectedAcademicUnitId]
  );
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const units = getData(unitRes);
      const sessionPayload = getData(sessionRes);
      ...
      if (!isFullAccess && restrictedUnitName) {
        ...
        if (allowedUnit) {
          setSelectedAcademicUnitId(allowedUnit._id);
        }
      }
    } catch (err) {
  ...
  const checkStatus = async () => {
    if (!selectedAcademicUnitId || !selectedSessionId || !selectedTermId) {
      toast.error("Please select academic unit, session and term");
      return;
    }
  ...
  const publishResult = async () => {
    ...
    const ok = window.confirm(
      "Publish this term result? Students in this academic unit will be able to view it."
    );
  ...
  const unpublishResult = async () => {
    ...
    const ok = window.confirm(
      "Unpublish this term result? Students in this academic unit will no longer be able to view it."
    );
  ...
  useEffect(() => {
    setStatus(null);
  }, [selectedAcademicUnitId, selectedTermId]);

  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6">
      <div className="mx-auto max-w-4xl">
        <div className="mb-4 rounded-xl bg-white p-4 shadow-sm">
          <h1 className="text-xl font-bold text-gray-900">Publish Results</h1>
          <p className="mt-1 text-sm text-gray-500">
            Control when students can view their term and yearly results.
          </p>
          ...
          {isAcademicUnitLocked && selectedAcademicUnit && (
            <p className="mt-3 rounded-lg bg-green-50 px-3 py-2 text-sm font-semibold text-green-700">
              Restricted View: {selectedAcademicUnit.name}
            </p>
          )}
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedAcademicUnitId(e.target.value)}
                disabled={loading || isAcademicUnitLocked}
                className="w-full rounded-lg border px-3 py-3 text-sm disabled:bg-gray-100"
              >
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                {status.publication?.publishedAt && (
                  <p className="mt-1 text-xs text-gray-500">
                    Published on{" "}
                    {new Date(status.publication.publishedAt).toLocaleString()}
                  </p>
                )}
              </div>
        ...
        <div className="mt-4 rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
          Students can only view a term result after that academic unit,
          session and term has been published. Yearly result becomes available
          only after all terms in the session are published.
        </div>
      </div>
    </div>
  );

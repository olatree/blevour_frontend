...
const FeeCollectionAnalysisPage = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [unitsRes, sessionsRes] = await Promise.all([
  ...
  const fetchAnalysis = async () => {
    try {
      ...
      const summaryRes = await api.get("/fees/reports/collection-summary", {
        params,
      });
  ...
  useEffect(() => {
    const run = async () => {
      await fetchClasses(filters.academicUnitId);
      setFilters((prev) => ({ ...prev, classId: "", armId: "" }));
    };
  ...
  const selectedFilterText = useMemo(() => {
    const unit = academicUnits.find((u) => u._id === filters.academicUnitId)?.name;
    const session = sessions.find((s) => s._id === filters.sessionId)?.name;
    const term = terms.find((t) => t._id === filters.termId)?.name;
    const cls = classes.find((c) => c._id === filters.classId)?.name;
    const arm = arms.find((a) => a._id === filters.armId)?.name;

    return [unit, session, term, cls, arm, filters.status]
      .filter(Boolean)
      .join(" · ");
  }, [filters, academicUnits, sessions, terms, classes, arms]);
    ...
    <div className="min-h-screen bg-slate-50 p-3 sm:p-5">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Monitor revenue, collections, and outstanding balances by academic unit.
          </p>
        </div>
        ...
        <button
          onClick={printReport}
          disabled={!hasSearched || !summary}
          className="self-start rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 print:hidden"
        >
          Print Report
        </button>
      </div>
      ...
      {error && (
        <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 print:hidden">
          {error}
        </div>
      )}
      ...
      <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm print:hidden">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
          <FilterSelect
            ...
            onChange={(value) =>
              setFilters({ ...filters, academicUnitId: value })
            }
          ...
          <FilterSelect
            label="Term"
            value={filters.termId}
            onChange={(value) => setFilters({ ...filters, termId: value })}
            options={terms}
            placeholder="All Terms"
          />
          ...
          <FilterSelect
            label="Arm"
            value={filters.armId}
            onChange={(value) => setFilters({ ...filters, armId: value })}
            options={arms}
            placeholder="All Arms"
          />
        ...
        <>
          <div className="mb-4 rounded-2xl bg-green-700 p-5 text-white shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-green-100">
              ...
              <span>
                Outstanding:{" "}
                <strong className="text-white">{outstandingRate}%</strong>
              </span>
              <span>
                Expected:{" "}
                <strong className="text-white">
                  {formatMoney(summary.totalExpected)}
                </strong>
              </span>
            </div>
          </div>
...
const SmallMetric = ({ label, value, sub, accent = "default" }) => (
  <div
    className={`rounded-xl border border-slate-200 border-l-4 bg-white p-3 shadow-sm ${accentMap[accent]}`}
  >
    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">
      {label}
    </p>

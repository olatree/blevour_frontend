...
const DebtorsListPage = () => {
  ...
  const saveFilters = (nextFilters, nextPage = page) => {
    sessionStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ filters: nextFilters, page: nextPage })
    );
  };
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [activeRes, unitsRes] = await Promise.all([
        api.get("/sessions/active"),
      ...
      const nextFilters = {
        academicUnitId: restoredFilters.academicUnitId || "",
        sessionId: activeSession?._id || "",
        termId: restoredFilters.termId || activeTerm?._id || "",
        classId: restoredFilters.classId || "",
        armId: restoredFilters.armId || "",
      ...
      await fetchClasses(nextFilters.academicUnitId);
  ...
  const selectedAcademicUnit = academicUnits.find(
    (unit) => unit._id === filters.academicUnitId
  );

  const selectedTerm = terms.find((term) => term._id === filters.termId);
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-6">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 sm:text-sm">
            Outstanding fee balances by academic unit, class, arm, and term.
          </p>
        </div>
      ...
      <div className="hidden print:block">
        ...
        <p className="mb-3 text-center text-xs">
          ...
          {selectedAcademicUnit ? ` • ${selectedAcademicUnit.name}` : ""}
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => updateFilter("academicUnitId", e.target.value)}
              className="w-full rounded-lg border px-3 py-3 text-sm outline-none focus:border-green-500"
            >
            ...
            <div className="hidden overflow-x-auto md:block print:block print:overflow-visible">
              <table className="min-w-full border text-sm">
                <thead className="bg-gray-100 text-left">
                  <tr>
                    <th className="border px-3 py-2">Student</th>
                    <th className="border px-3 py-2">Academic Unit</th>
                    <th className="border px-3 py-2">Class</th>
                    <th className="border px-3 py-2">Session/Term</th>
                    <th className="border px-3 py-2">Payable</th>
                    <th className="border px-3 py-2">Paid</th>
...
const DebtorCard = ({ account }) => (
  <div className="rounded-xl border bg-white p-3 text-sm">
    <div className="mb-3">
      <h3 className="font-semibold text-gray-800">{account.studentId?.name}</h3>
      <p className="text-xs text-gray-500">
        Admission No: {account.studentId?.admissionNumber}
      </p>
      <p className="text-xs text-gray-500">
    ...
    <div className="mb-3 rounded-lg bg-gray-50 p-3">
      <p className="text-xs text-gray-500">Academic Unit</p>
      <p className="text-sm font-medium text-gray-800">
        {account.academicUnitId?.name || "N/A"}
      </p>

      <p className="mt-2 text-xs text-gray-500">Class</p>
      <p className="text-sm font-medium text-gray-800">
        {account.classId?.name} {account.armId?.name}
      </p>

...
const EditLessonPage = () => {
  ...
  const selectedAcademicUnitObj = useMemo(
    () => academicUnits.find((unit) => unit._id === form.academicUnitId),
    [academicUnits, form.academicUnitId]
  );

  const fetchAcademicUnits = async () => {
    const unitRes = await api.get("/academic-units");
    const unitPayload = getData(unitRes);
    return Array.isArray(unitPayload) ? unitPayload : [];
  };
  ...
  const fetchClassesAndSubjects = async (academicUnitId) => {
    ...
    const classPayload = getData(classesRes);
    const subjectPayload = getData(subjectsRes);
  ...
  const fetchData = async () => {
    try {
      ...
      const [lessonRes, units] = await Promise.all([
        api.get(`/lessons/${id}`),
        fetchAcademicUnits(),
      ]);
      ...
      const lessonAcademicUnitId =
        lessonData.academicUnitId?._id || lessonData.academicUnitId || "";

      let allowedAcademicUnitId = lessonAcademicUnitId;
      ...
      if (!isFullAccess && restrictedUnitName) {
        ...
        allowedAcademicUnitId = allowedUnit?._id || "";

        if (
          lessonAcademicUnitId &&
          allowedAcademicUnitId &&
          lessonAcademicUnitId !== allowedAcademicUnitId
        ) {
          setError("You are not allowed to edit this lesson.");
          return;
        }
      }

      await fetchClassesAndSubjects(allowedAcademicUnitId);

      setForm({
        academicUnitId: allowedAcademicUnitId,
        title: lessonData.title || "",
        description: lessonData.description || "",
        week: lessonData.week || 1,
        classId: lessonData.classId?._id || lessonData.classId || "",
  ...
  useEffect(() => {
    if (!form.academicUnitId) return;

    fetchClassesAndSubjects(form.academicUnitId).catch((err) => {
      setError(
      ...
      >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Select
            ...
            onChange={(value) => updateForm("academicUnitId", value)}
            options={visibleAcademicUnits}
...
const ResourceInputBlock = ({ title, items, updateItem, addItem, fields }) => (
  <div className="mb-5 rounded-lg border p-3">
    <div className="mb-3 flex items-center justify-between">
      <h3 className="font-semibold text-gray-800">{title}</h3>
      <button

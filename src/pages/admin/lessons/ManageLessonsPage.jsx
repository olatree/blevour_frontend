...
const ManageLessonsPage = () => {
  ...
  const selectedAcademicUnitObj = useMemo(
    () => academicUnits.find((unit) => unit._id === filters.academicUnitId),
    [academicUnits, filters.academicUnitId]
  );
  ...
  useEffect(() => {
    fetchClassesAndSubjects(filters.academicUnitId);
  ...
  useEffect(() => {
    if (activeSessionTerm?.session?._id && filters.academicUnitId) {
      fetchLessons();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filters.academicUnitId]);
  ...
  const resetFilters = () => {
    setFilters((prev) => ({
      ...prev,
      academicUnitId: isAcademicUnitLocked ? prev.academicUnitId : "",
      termId: activeSessionTerm?.term?._id || "",
      classId: "",
      armId: "",
      subjectId: "",
  ...
  const archiveLesson = async (lesson) => {
    ...
    try {
      ...
      setMessage("Lesson archived successfully");
      fetchLessons();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to archive lesson");
    }
      ...
      <div className="mb-5 rounded-2xl bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-7">
          <Select
            ...
            onChange={(value) => updateFilter("academicUnitId", value)}
          ...
          <Select
            ...
            options={classes}
            placeholder="All Classes"
            disabled={!filters.academicUnitId}
          />
